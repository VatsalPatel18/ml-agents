...
# --- Agent Callbacks ---
def log_before_agent(callback_context: CallbackContext, **kwargs) -> None:
...
def log_after_agent(callback_context: CallbackContext, **kwargs) -> None:
    """Logs after an agent's run method finishes."""
    agent_name = callback_context.agent_name
    invocation_id = callback_context.invocation_id
    agent_flow_logger.info(
        f"INVOKE_ID={invocation_id}: <--- Exiting Agent '{agent_name}'"
    )

# --- Tool Callbacks ---
def log_before_tool(tool: BaseTool, args: Dict[str, Any], tool_context: ToolContext, **kwargs) -> Optional[Dict]:
...
def log_after_tool(tool: BaseTool, args: Dict[str, Any], tool_context: ToolContext, result: Optional[Any], **kwargs) -> Optional[Dict]:
    ...
    # Artifact saving logic for code_execution_tool
    if tool_name == "code_execution_tool" and status == "success":
        ...
        if plot_info and isinstance(plot_info, dict) and isinstance(result, dict):
            ...
            if plot_logical_name and plot_output_key in output_files:
                ...
                tool_calls_logger.info(
                    f"INVOKE_ID={invocation_id}: Found expected plot output '{plot_output_key}' at '{plot_local_path}'. Attempting artifact save."
                )
                try:
                    from core_tools.artifact_helpers import save_plot_artifact
                    artifact_name = save_plot_artifact(plot_local_path, plot_logical_name, tool_context)
                    if artifact_name:
                        state_key_base = plot_info.get("state_key_base")
                        if state_key_base:
                            tool_calls_logger.info(
                                f"INVOKE_ID={invocation_id}: Plot artifact '{artifact_name}' saved. "
                                f"Agent should update state key like '{state_key_base}.plots'."
                            )
                        else:
                            tool_calls_logger.warning(
                                f"INVOKE_ID={invocation_id}: Plot artifact saved ('{artifact_name}'), "
                                "but no state_key_base provided."
                            )
                    if "temp:expected_plot_output" in tool_context.state:
                        tool_context.state.pop("temp:expected_plot_output")
                except ImportError:
                    tool_calls_logger.error(
                        f"INVOKE_ID={invocation_id}: Could not import artifact_helpers in callback."
                    )
                except Exception as e:
                    tool_calls_logger.error(
                        f"INVOKE_ID={invocation_id}: Error saving artifact in callback: {e}"
                    )
